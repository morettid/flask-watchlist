...
from movie_library.forms import ExtendMovieForm, LoginForm, MovieForm, RegisterForm
...
# decorator to allow access only if the email is registred in the session!
...
def register():
    ...
    if form.validate_on_submit():
        ...
        flash("User registred succesfully", "success")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user_data:
            flash("Login credentials data not correct.", category="danger")
            return redirect(url_for('.login'))
    ...
    return render_template(
        "login.html", title="Movies Watchlist - Login", form=form
    )
...
def edit_movie(_id: str):
    ...
    form = ExtendMovieForm(obj=movie)
    ...
    return render_template(
        "movie_form.html", title="Movies watchlist - Add Movie", form=form
    )
...
# to do:
# 1. a list of all unique films added in the database
# 2. average the rates, allow for comments/reviews